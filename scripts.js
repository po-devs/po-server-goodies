...
beforeChangeTier : function(src, team, oldtier, newtier) {
    sendChanMessage(sys.id('[LD]Jirachier'), src+" above the if", 2);
	sendChanMessage(sys.id('Crystal Moogle'), src+" above the if", 2);
    if (!tier_checker.has_legal_team_for_tier(src, team, newtier)) {
	   sendChanMessage(sys.id('[LD]Jirachier'), src+" inside the if", 2);
	   sendChanMessage(sys.id('Crystal Moogle'), src+" inside the if", 2);
       sys.stopEvent();
       normalbot.sendMessage(src, "Sorry, you can not change into that tier.");
       tier_checker.find_good_tier(src, team);
    }
	sendChanMessage(sys.id('[LD]Jirachier'), src+" under the if", 2);
	sendChanMessage(sys.id('Crystal Moogle'), src+" under the if", 2);
	// PO logs stuff
