...
require = function require(module_name, retry) {
    if (require.cache[module_name]) return require.cache[module_name];
    ...
    with(module) {
        ...
        if (content) {
            try {
                ...
                if (staffchannel) sys.sendAll("Error loading module " + module_name + ": " + e + (e.lineNumber ? " on line: " + e.lineNumber : ""), staffchannel);
                else sys.sendAll("Error loading module " + module_name + ": " + e);
...
var updateModule = function updateModule(module_name, callback) {
    ...
    var url;
    if (/^https?:\/\//.test(module_name)) url = module_name;
    else url = base_url + "scripts/" + module_name;
...
sys.makeDir("scripts"); /* we need to make sure the scripts exist */
...
var cleanFile = function (filename) {
    if (typeof sys != 'undefined') sys.appendToFile(filename, "");
};
...
if (typeof SESSION.global() != 'undefined') {
    ...
    sys.channelIds().forEach(function (id) {
        if (!SESSION.channels(id)) sys.sendAll("ScriptUpdate: SESSION storage broken for channel: " + sys.channel(id), staffchannel);
        else SESSION.channels(id).__proto__ = POChannel.prototype;
    });
...
poScript = ({ /* Executed every second */
    step: function () {
    ...
    init: function () {
        ...
        for (dwpok = 0; dwpok < halist.length; dwpok++) {
            ...
            if (num === undefined) sys.sendAll("Script Check: Unknown poke in hapokemons: '" + halist[dwpok] + "'.", announceChan);
            else if (script.hapokemons[num] === true) sys.sendAll("Script Check:  hapokemons contains '" + halist[dwpok] + "' multiple times.", announceChan);
            else {
    ...
    "- Ignorance of the rules is not a valid reason for breaking them. Do not attempt to find or create any loopholes in these rules, or try to adapt them in order to have a punishment overturned or to justify your actions. Doing so may incur a further punishment. Make valid appeals directly to the authority of the server."
        ...
        if (typeof script.authStats == 'undefined') script.authStats = {};
        ...
        isSuperAdmin = function (id) {
            ...
            for (var i = 0; i < Config.superAdmins.length; ++i) {
                if (script.cmp(name, Config.superAdmins[i])) return true;
            }
            return false;
        };

        if (typeof VarsCreated != 'undefined') return;
    ...
    issueBan: function (type, src, tar, commandData, maxTime) {
        ...
        sys.playerIds().forEach(function (id) {
            if (sys.loggedIn(id) && sys.ip(id) === tarip) SESSION.users(id).activate(type, sys.name(src), expires, reason, true);
        });
        if (!sys.loggedIn(tar)) {
    ...
    banList: function (src, command, commandData) {
        ...
        for (var ip in mh.hash) {
            if (mh.hash.hasOwnProperty(ip)) {
                ...
                if (typeof commandData != 'undefined' && (!banned_name || banned_name.toLowerCase().indexOf(commandData.toLowerCase()) == -1)) continue;
        ...
        table += table_footer;
        if (send_rows > 0) sys.sendHtmlMessage(src, table, channel);
        return;
    },
...
*/
        ...
        for (var i = 0; i < 6; ++i) {
            ...
            if (!compactible && ivs.length > 0) ret.push('IVs: ' + ivs.join(" / "));
            if (evs.length > 0) ret.push('EVs: ' + evs.join(" / "));
    ...
    canJoinStaffChannel: function (src) {
        ...
        if (disallowedNames.indexOf(sys.name(src)) > -1) return false;
        if (sys.auth(src) > 0) return true;
        if (SESSION.users(src).megauser) return true;
        if (SESSION.users(src).contributions !== undefined) return true;
        var allowedNames = Config.canJoinStaffChannel;
        if (allowedNames.indexOf(sys.name(src)) > -1) return true;
        return false;
    },

    isOfficialChan: function (chanid) {
        var officialchans = [0, tourchannel, mafiachan, triviachan, hangmanchan];
        if (officialchans.indexOf(chanid) > -1) return true;
        else return false;
    },
...
/*if (SESSION.channels(chan).topicSetter)
    ...
    afterNewMessage: function (message) {
        if (message == "Script Check: OK") {
            ...
            if (typeof(scriptChecks) == 'undefined') scriptChecks = 0;
            scriptChecks += 1;
            this.init();
        }
    ...
    afterLogIn: function (src) {
        ...
        if (typeof(this.startUpTime()) == "string") countbot.sendMessage(src, "Server uptime is " + this.startUpTime());
        sys.sendMessage(src, "");
        ...
        if (SESSION.users(src).megauser) sys.appendToFile("staffstats.txt", sys.name(src) + "~" + src + "~" + sys.time() + "~" + "Connected as MU" + "\n");
        if (sys.auth(src) > 0 && sys.auth(src) <= 3) sys.appendToFile("staffstats.txt", sys.name(src) + "~" + src + "~" + sys.time() + "~" + "Connected as Auth" + "\n");
        authChangingTeam = (sys.auth(src) > 0 && sys.auth(src) <= 3);
        this.afterChangeTeam(src);

        if (sys.auth(src) <= 3 && this.canJoinStaffChannel(src)) sys.putInChannel(src, staffchannel);
...
/*
    ...
    */
    ...
    beforeLogOut: function (src) {
        if (SESSION.users(src).megauser) sys.appendToFile("staffstats.txt", sys.name(src) + "~" + src + "~" + sys.time() + "~" + "Disconnected as MU" + "\n");
        if (sys.auth(src) > 0 && sys.auth(src) <= 3) sys.appendToFile("staffstats.txt", sys.name(src) + "~" + src + "~" + sys.time() + "~" + "Disconnected as Auth" + "\n");
    },
    ...
    afterChangeTeam: function (src) {
        ...
        if (authChangingTeam === false) {
            if (sys.auth(src) > 0 && sys.auth(src) <= 3) sys.appendToFile("staffstats.txt", sys.name(src) + "~" + src + "~" + sys.time() + "~" + "Changed name to Auth" + "\n");
        } else if (authChangingTeam === true) {
            if (!(sys.auth(src) > 0 && sys.auth(src) <= 3)) sys.appendToFile("staffstats.txt", "~" + src + "~" + sys.time() + "~" + "Changed name from Auth" + "\n");
        }
        ...
        for (var team = 0; team < sys.teamCount(src); team++) {
            try {
                ...
                if (sys.gen(src, team) === 2) {
                    ...
                    if (sys.hasTeamPokeMove(src, team, i, bannedGSCSleep[j])) for (var k = 0; k < bannedGSCTrap.length; ++k) if (sys.hasTeamPokeMove(src, team, i, bannedGSCTrap[k])) {
        ...
        if (!chanName) {
            ...
            if (cid !== undefined) {
                ...
                sys.delayedCall(function () {
                    if (!SESSION.channels(cid).muteall) return;
    ...
    beforeChatMessage: function (src, message, chan) {
        ...
        if (message.substr(0, 1) == '%') {
            if (sys.id('JiraBot') !== undefined) sys.sendMessage(sys.id('JiraBot'), sys.name(src) + ": " + message, chan);
            if (sys.id('PolkaBot') !== undefined) sys.sendMessage(sys.id('PolkaBot'), sys.name(src) + ": " + message, chan);
            sys.stopEvent();
            return;
        }
        channel = chan;
        if ((script.isOfficialChan(chan) && message.length > 250 && sys.auth(src) < 1) || (message.length > 5000 && sys.auth(src) < 2)) {
...
/*var shanaiForward = function(msg) {
    ...
    afterChatMessage: function (src, message, chan) {
        ...
        if (!poChannel.ignorecaps && this.isMCaps(message) && userMayGetPunished && !capsday) {
            ...
            if (user.caps >= maxCaps && !user.mute.active) {
                ...
                if (user.capsmutes === undefined) user.capsmutes = 0;
                ...
                if (officialChan) {
                    ...
                    if (user.smute.active) {
                        ...
                        if (channel != staffchannel) capsbot.sendAll(message + " [Channel: " + sys.channel(channel) + "]", staffchannel);
                        if (channel != watchchannel) capsbot.sendAll(message + " [Channel: " + sys.channel(channel) + "]", watchchannel);
                    }
                }
            ...
            if (user.floodcount > linecount) {
                ...
                if (officialChan) {
                    if (user.smute.active) {
                        ...
                        if (channel != staffchannel) kickbot.sendAll(message + ". [Channel: " + sys.channel(channel) + "]", staffchannel);
                        if (channel != watchchannel) kickbot.sendAll(message + ". [Channel: " + sys.channel(channel) + "]", watchchannel);
                    }
                }
                if (officialChan) {
                    if (sys.auth(src) === 0) {
    ...
    afterBattleStarted: function (src, dest, clauses, rated, mode, bid, team1, team2) {
        ...
        var time = parseInt(sys.time(), 10);
        var battle_data = {
            players: [sys.name(src), sys.name(dest)],
            clauses: clauses,
            rated: rated,
            mode: mode,
            tier: tier,
...
/*
    ...
    */
    ...
    beforeChallengeIssued: function (src, dest, clauses, rated, mode, team, destTier) {
        ...
        var isChallengeCup = /*sys.getClauses(sys.tier(src,team))%32 >= 16 ||*/
        sys.getClauses(destTier) % 32 >= 16;
