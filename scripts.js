...
var Config = {
    base_url: "https://raw.github.com/coyotte508/po-server-goodies/master/",
    dataDir: "scriptdata/",
    bot: "Dratini",
    kickbot: "Blaziken",
    capsbot: "Exploud",
...
autosmute = sys.getFileContent("secretsmute.txt").split(':::');
crc32 = require('crc32.js').crc32;
MemoryHash = require('memoryhash.js').MemoryHash;
delete require.cache['tierchecks.js'];
tier_checker = require('tierchecks.js');

commands = {};
for (var i = 0; i < commandfiles.length; i++) {
    push_properties(require(commandfiles[i]), commands);
}
...
Lazy = utilities.Lazy;
nonFlashing = utilities.non_flashing;
...
POChannel.prototype.setTopic = function(src, topicInfo)
{
    ...
    if (topicInfo === undefined) {
        if (typeof this.topic != 'undefined') {
            bots.channel.sendChanMessage(src, "Topic for this channel is: " + this.topic);
            if (SESSION.channels(channel).topicSetter) {
                bots.channel.sendChanMessage(src, "Topic was set by " + nonFlashing(this.topicSetter));
            }
        } else {
            bots.channel.sendChanMessage(src, "No topic set for this channel.");
        }
        if (canSetTopic) {
            bots.channel.sendChanMessage(src, "Specify a topic to set one!");
        }
        return;
    }
    if (!canSetTopic) {
        bots.channel.sendChanMessage(src, "You don't have the rights to set topic");
        return;
    }
    this.changeParameter(src, "topic", topicInfo);
    SESSION.global().channelManager.update(this.id);
    bots.channel.sendChanAll("" + sys.name(src) + " changed the topic to: " + topicInfo);
};
...
POChannel.prototype.issueAuth = function(src, name, group)
{
    ...
    if (ret[0] == "self") {
        bots.channel.sendChanMessage(src, ret[1]);
    }
    else {
        bots.channel.sendChanAll(ret[1]);
        SESSION.global().channelManager.update(this.id);
    }
};
    ...
    if (ret[0] == "self") {
        ...
        if (sys.id(tar) !== undefined) {
            if (sys.isInChannel(sys.id(tar), this.id)) {
                ...
                bots.channel.sendChanAll("And "+tar+" was gone!");
            }
        }
    }
};
    ...
    for (var x in mutelist) {
        if (mutelist.hasOwnProperty(x)) {
            ...
            if (mutelist[x].expiry <= parseInt(sys.time(),10)) {
                delete this.muted[x];
                bots.channel.sendChanAll(x+"'s channel mute expired.");
                continue;
            }
            if (cmp(x, name)) {
                return true;
...
bots = {
    'normal': normalbot, 'mafia': mafiabot, 'channel': channelbot,
    'kick': kickbot, 'caps': capsbot, 'check': checkbot,
    'coin': coinbot, 'count': countbot, 'tourney': tourneybot,
    'ranking': rankingbot, 'battle': battlebot, 'command': commandbot,
...
init : function() {
    ...
    try {
        ...
        bots.normal.sendAll("Couldn't load api keys: " + e, staffchannel);
    }
...
beforeChannelJoin : function(src, channel) {
    ...
    if (sys.auth(src) < 3 && poChannel.canJoin(src) == "banned") {
        bots.channel.sendMessage(src, "You are banned from this channel! You can't join unless channel operators and masters unban you.");
        sys.stopEvent();
        return;
    }
    ...
    for(var x = 0; x < channels.length; x++) {
        if (channel == channels[x] && poUser[bans[x]].active) {
            if (poUser.expired(bans[x])) {
                poUser.un(bans[x]);
                bots.normal.sendMessage(src, "Your ban from " + type[x] + " expired.");
            } else {
...
beforePlayerBan : function(src, dest, dur) {
    bots.normal.sendAll("Target: " + sys.name(dest) + ", IP: " + sys.ip(dest), staffchannel);
...
beforeLogIn : function(src) {
    ...
    if (this.isRangeBanned(ip) && allowedIps.indexOf(ip) == -1) {
        bots.normal.sendMessage(src, 'You are banned!');
        sys.stopEvent();
        return;
    }
    if (proxy_ips.hasOwnProperty(ip)) {
        bots.normal.sendMessage(src, 'You are banned for using proxy!');
        sys.stopEvent();
        return;
...
nameIsInappropriate: function(src)
{
    var name = (typeof src == "number")
        ...
        : src;
    function reply(m) {
       if (typeof src == "number") bots.normal.sendMessage(src, m);
    }
...
afterLogIn : function(src) {
    ...
    bots.command.sendMessage(src, "Use !commands to see the commands!");
    ...
    bots.count.sendMessage(src, "Max number of players online was " + sys.getVal("MaxPlayersOnline") + ".");
    if (typeof(this.startUpTime()) == "string")
    bots.count.sendMessage(src, "Server uptime is "+this.startUpTime());
    sys.sendMessage(src, "");
...
//    }
    ...
    if (isAndroid(src)) {
        bots.normal.sendMessage(src, "Give suggestions for the Android Teambuilder here: http://goo.gl/Bois5");
        bots.normal.sendMessage(src, "Remember to report any android teambuilder related bugs at: http://pokemon-online.eu/forums/showthread.php?20883-TBuilder-for-Android");
    }
}, /* end of afterLogin */
...
beforePlayerRegister : function(src) {
    if (sys.name(src).match(/\bguest[0-9]/i)) {
        sys.stopEvent();
        bots.normal.sendMessage(src, "You cannot register guest names!");
        return;
    }
    var limit = Config.registeredLimit;
    if (limit > 0 && sys.numRegistered(sys.ip(src)) >= limit && sys.auth(src) === 0) {
        sys.stopEvent();
        bots.normal.sendMessage(src, "You cannot register more than " + limit + " names! Use /myalts to get a list of your alts.");
        return;
    }
},
...
afterChangeTeam : function(src)
{
    ...
    for (var team = 0; team < sys.teamCount(src); team++) {
                        ...
                        if (sys.hasTeamPokeMove(src, team, i, bannedGSCSleep[j]))
                            for (var k = 0; k < bannedGSCTrap.length; ++k)
                                if (sys.hasTeamPokeMove(src, team, i, bannedGSCTrap[k])) {
                                    bots.check.sendMessage(src, "SleepTrapping is banned in GSC. Pokemon " + sys.pokemon(sys.teamPoke(src,team,i)) + "  removed from your team.");
        ...
        if (!tier_checker.has_legal_team_for_tier(src, team, sys.tier(src, team))) {
            ...
            bots.normal.sendMessage(src, "You were placed into '" + sys.tier(src, team) + "' tier.");
        }
    }
...
silence: function(src, minutes, chanName) {
    ...
    if (isNaN(delay) || delay <= 0) {
        bots.channel.sendChanMessage(src, "Sorry, I couldn't read your minutes.");
    }
    if (!chanName) {
        ...
        if (cid !== undefined) {
            bots.channel.sendAll("" + sys.name(src) + " called for " + minutes + " Minutes Of Silence in "+chanName+"!", cid);
            SESSION.channels(cid).muteall = true;
            sys.delayedCall(function() {
                if (!SESSION.channels(cid).muteall)
                    return;
                SESSION.channels(cid).muteall = false;
                bots.normal.sendAll("Silence is over in "+chanName+".",cid);
            }, delay);
        } else {
            bots.channel.sendChanMessage(src, "Sorry, I couldn't find a channel with that name.");
        }
    }
},
...
silenceoff: function(src, chanName) {
    if (chanName !== undefined) {
        var cid = sys.channelId(chanName);
        if (!SESSION.channels(cid).muteall) {
            bots.channel.sendChanMessage(src, "The channel is not muted.");
            return;
        }
        bots.channel.sendAll("" + sys.name(src) + " cancelled the Minutes of Silence in "+chanName+"!", cid);
        SESSION.channels(cid).muteall = false;
    } else {
        bots.normal.sendChanMessage("Use /silenceoff Channel Name");
    }
},
...
meoff: function(src, commandData) {
    ...
    if (cid !== undefined) {
        ...
        bots.normal.sendAll("" + sys.name(src) + " turned off /me in "+commandData+".", cid);
    } else {
        bots.normal.sendChanMessage(src, "Sorry, that channel is unknown to me.");
    }
    return;
},
...
meon: function(src, commandData) {
    ...
    if (cid !== undefined) {
        ...
        bots.normal.sendAll("" + sys.name(src) + " turned on /me in "+commandData+".", cid);
    } else {
        bots.normal.sendChanMessage(src, "Sorry, that channel is unknown to me.");
    }
},
...
beforeNewPM: function(src){
    ...
    if (user.pmcount > pmlimit){
        ...
        if (user.pmwarned === false) {
            bots.normal.sendAll('User ' + sys.name(src) + ' is potentially spamming through PM', sys.channelId('Indigo Plateau'));
            user.pmwarned = true;
        }
        return;
    }
...
beforeChatMessage: function(src, message, chan) {
    ...
    if ((chan === 0 && message.length > 250 && sys.auth(src) < 1)
       || (message.length > 5000 && sys.auth(src) < 2)) {
        bots.normal.sendChanMessage(src, "Hi! Your message is too long, please make it shorter :3");
        sys.stopEvent();
        return;
    }
    ...
    if (channel === 0 && sys.auth(src) === 0) {
        ...
        if (points >= 5) {
            bots.normal.sendAll('Spammer: "' + sys.name(src) + '", ip: ' + sys.ip(src) + ', message: "' + message + '". Banned.', staffchannel);
    ...
    if (SESSION.users(src).expired("mute")) {
        ...
        bots.normal.sendChanMessage(src, "your mute has expired.");
    }
    if (sys.auth(src) < 3 && SESSION.users(src).mute.active && message != "!join" && message != "/rules" && message != "/join" && message != "!rules") {
        ...
        bots.normal.sendChanMessage(src, "You are muted" + (muteinfo.by ? " by " + muteinfo.by : '')+". " + (muteinfo.expires > 0 ? "Mute expires in " + getTimeString(muteinfo.expires - parseInt(sys.time(), 10)) + ". " : '') + (muteinfo.reason ? "[Reason: " + muteinfo.reason + "]" : ''));
        sys.stopEvent();
        return;
    }
    var poChannel = SESSION.channels(channel);
    if (sys.auth(src) < 1 && !poChannel.canTalk(src)) {
        bots.channel.sendChanMessage(src, "You are muted on this channel! You can't speak unless channel operators and masters unmute you.");
        sys.stopEvent();
        return;
    }
    ...
    repeatingOneself = new Lazy(function() {
        ...
        if (!SESSION.channels(channel).isChannelOperator(src) && SESSION.users(src).contributions === undefined && sys.auth(src) < 1 && user.lastline.message == message && user.lastline.time + 15 > time) {
            bots.normal.sendChanMessage(src, "Please do not repeat yourself!");
            ret = true;
        }
        user.lastline.time = time;
        user.lastline.message = message;
    ...
    if ((message[0] == '/' || message[0] == "!") && message.length > 1 && utilities.isLetter(message[1])) {
        ...
        commands.handleCommand(src, command, commandData, tar);
    ...
    if (SESSION.channels(channel).muteall && !SESSION.channels(channel).isChannelOperator(src) && sys.auth(src) === 0) {
        bots.normal.sendChanMessage(src, "Respect the minutes of silence!");
        sys.stopEvent();
        return;
    }
    ...
    if (usingBannedWords()) {
        if (message.indexOf(".tk") != -1){
            bots.normal.sendAll(sys.name(src) + " tried to send a .tk link!",staffchannel);
        }
    ...
    if (capsName() && !capsday) {
        bots.normal.sendChanMessage(src, "You have too many CAPS letters in your name. Please remove them to speak freely. 7 CAPS letters are allowed. Lowercase name will keep your ladder score.");
        sys.stopEvent();
        return;
    }
    ...
    if (channel === 0 && typeof clanmute != 'undefined') {
       ...
       if (bracket1 >= 0 && bracket2 > 0 && bracket1 < bracket2) {
           bots.normal.sendMessage(src, "Sorry, clan members can't speak on the main chat.");
           sys.stopEvent();
           return;
       }
...
afterChatMessage : function(src, message, chan)
{
   ...
   // if (channel == sys.channelId("PO Android")) {
       // if (/f[uo]ck|\bass|\bcum|\bdick|\bsex|pussy|bitch|porn|\bfck|nigga|\bcock|\bgay|\bhoe\b|slut|whore|cunt|clitoris/i.test(message) && user.android) {
           // bots.kick.sendAll(sys.name(src) + " got kicked for foul language.", channel);
           // sys.kick(src);
           // return;
       // }
   // }
        ...
        if (user.caps >= maxCaps && !user.mute.active) {
            ...
            if (officialChan) {
                ...
                if (user.smute.active) {
                    ...
                    bots.caps.sendAll("" + sys.name(src) + " was muted for caps while smuted.", staffchannel);
                } else {
                    bots.caps.sendChanAll(message);
                    if (channel != staffchannel)
                        bots.caps.sendAll(message + "[Channel: "+sys.channel(channel) + "]", staffchannel);
                }
            }
    ...
    if (!poChannel.ignoreflood && userMayGetPunished) {
        ...
        if (user.floodcount > linecount) {
            ...
            if (officialChan) {
                if (user.smuted) {
                    sys.sendMessage(src, message);
                    bots.kick.sendAll("" + sys.name(src) + " was kicked for flood while smuted.", staffchannel);
                } else {
                    bots.kick.sendChanAll(message);
                    if (channel != staffchannel)
                        bots.kick.sendAll(message + " [Channel: "+sys.channel(channel)+"]", staffchannel);
                }
            }
            if (officialChan) {
                if (sys.auth(src) === 0) {
...
beforeChangeTier : function(src, team, oldtier, newtier) {
    ...
    if (!tier_checker.has_legal_team_for_tier(src, team, newtier)) {
       sys.stopEvent();
       bots.normal.sendMessage(src, "Sorry, you can not change into that tier.");
       tier_checker.find_good_tier(src, team);
    }
},
...
beforeChallengeIssued : function (src, dest, clauses, rated, mode, team, destTier) {
    if (battlesStopped) {
        bots.battle.sendMessage(src, "Battles are now stopped as the server will restart soon.");
        sys.stopEvent();
        return;
    }

    if (SESSION.users(dest).sametier === true && (destTier != sys.tier(src,team))) {
        bots.battle.sendMessage(src, "That guy only wants to fight his/her own tier.");
        sys.stopEvent();
        return;
    }
    ...
    if (isChallengeCup && !hasChallengeCupClause) {
        bots.check.sendMessage(src, "Challenge Cup must be enabled in the challenge window for a CC battle");
        sys.stopEvent();
        return;
    }
    /* Oak's request
    else if (!isChallengeCup && hasChallengeCupClause) {
        bots.check.sendMessage(src, "Challenge Cup must not be enabled in the challenge window for a non CC battle");
        sys.stopEvent();
        return;
    }*/

    if (sys.tier(src,team).indexOf("Doubles") != -1 && destTier.indexOf("Doubles") != -1 && mode != 1) {
        bots.battle.sendMessage(src, "To fight in doubles, enable doubles in the challenge window!");
        sys.stopEvent();
        return;
    }

    if (sys.tier(src,team).indexOf("Triples") != -1 && destTier.indexOf("Triples") != -1 && mode != 2) {
        bots.battle.sendMessage(src, "To fight in triples, enable triples in the challenge window!");
        sys.stopEvent();
        return;
    }
