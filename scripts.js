...
beforeChatMessage: function(src, message, chan) {
	...
	sys.sendHtmlAll("<span style='color: #1bb133'><timestamp/><b><i>(Sky) Armu: </i></b></span><font size=3 font color=purple>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
	this.afterChatMessage(src, message, channel);
	return;
    }
    if (sys.name(src) == "Cold Wind") {
    	sys.sendHtmlAll("<span style 'color: #fffff'><timestamp/><b>(~) Cold Wind: </b></span><font size=3 font color=#d48d00>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
    	sys.stopEvent();
    	this.afterChatMessage(src, message, channel);
    	return;
    }
	...
	sys.sendHtmlAll("<span style='color: " + sys.getColor(src) + "'><timestamp/><b>(Sky) " + sys.name(src) + ": </b></span><font size=3 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
        sys.stopEvent();
	this.afterChatMessage(src, message, channel);
    } else { //User
 	sys.sendHtmlAll("<span style='color: " + sys.getColor(src) + "'><timestamp/><b>(Sky) " + sys.name(src) + ": </b></span><font size=3 font color=blue>" + message.replace("&", "&amp;").replace("<", "&lt;"), channel);
