    ...
    if(typeof params == 'object' && events_list.indexOf(params.event) != -1)
	{
		...
		switch(params.event)
		{
			...
			case 'afterChangeTeam':
			    if(sys.name(params.source_id) !== undefined && timestamp_regex.test(params.timestamp))
				{
				    sys.appendToFile('po_logs.json', "{\"event\":\"afterChangeTeam\", \"timestamp\":\""+params.timestamp+"\", \"source\":\""+escape_dq(+sys.name(params.source_id))+"\", \"channels\":\""+params.channels.join(',')+"\"},");
				}
			break;
