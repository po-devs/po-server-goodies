...
if (typeof Config == "undefined") {
    Config = {
        DreamWorldTiers: ["No Preview OU", "No Preview Ubers", "X/Y", "Black/White", "Black/White 2"]
    };
}
...
tier_checker.add_new_check(INCLUDING, ["ORAS Doubles", "ORAS Triples", "ORAS Ubers", "ORAS OU", "ORAS UU", "ORAS LU", "ORAS NU", "ORAS LC", "Inverted Battle", "ORAS 1v1", "XY Ubers", "ORAS Cup", "XY OU", "XY UU", "XY LC", "No Preview OU", "BW2 OU", "BW2 UU", "BW2 LU", "BW2 LC", "DW LC", "BW2 Ubers", "No Preview Ubers", "Clear Skies", "Clear Skies DW", "Monotype", "Monocolour", "Monogen", "Smogon OU", "Smogon UU", "Smogon RU", "BW2 NU", "Metronome", "BW2 NEU"], function endlessCheck(src, team, tier) {
    var ret = [];
    for (var i = 0; i < 6; i++) {
...
tier_checker.add_new_check(EXCLUDING, [], function eventShinies(player, team) {
    ...
    if (sys.teamPoke(player, team, slot) == beast) for (var i = 0; i < 4; i++) if (-1 != beasts[beast].indexOf(sys.teamPokeMove(player, team, slot, i))) sys.changePokeShine(player, team, slot, true);
});

tier_checker.add_new_check(EXCLUDING, challenge_cups, function hasOneUsablePokemon(player, team) {
    for (var slot = 0; slot < 6; slot++)
    if (sys.teamPoke(player, team, slot) !== 0) for (var move = 0; move < 4; move++) if (sys.teamPokeMove(player, team, slot, move) !== 0) return;
