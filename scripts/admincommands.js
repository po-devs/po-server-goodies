...
exports.handleCommand = function (src, command, commandData, tar, channel) {
    ...
    if (command === "indigodeinvite") {
        ...
        for (x = 0; x < playersLength; x++) {
            var currentPlayer = players[x];
            if (sys.isInChannel(currentPlayer, staffchannel) && !script.canJoinStaffChannel(currentPlayer)) {
                sys.kick(currentPlayer, staffchannel);
                SESSION.channels(channel).takeAuth(src, sys.name(currentPlayer), "member");
                count = 1;
            }
        }
    ...
    if (command === "unban") {
        ...
        var x, banList = sys.banList();
        for (x in banList) {
            if (sys.dbIp(commandData) === sys.dbIp(banList[x])) {
    ...
    if (command === "nameban") {
        ...
        try {
            ...
            normalbot.sendMessage(src, "Sorry, your regular expression '" + commandData + "' fails. (" + error + ")", channel);
        }
        nameBans.push(regex);
        var x, serialized = {nameBans: []};
        for (x = 0; x < nameBans.length; x++) {
            serialized.nameBans.push(nameBans[x].source);
        }
    ...
    if (command === "channameban" || command === "channelnameban") {
        ...
        var x, serialized = {chanNameBans: []};
        for (x = 0; x < script.chanNameBans.length; x++) {
            serialized.chanNameBans.push(script.chanNameBans[x].source);
        }
    ...
    if (command === "namewarn") {
        ...
        var x, serialized = {nameWarns: []};
        for (x = 0; x < nameWarns.length; x++) {
            serialized.nameWarns.push(nameWarns[x].source);
        }
        ...
        if (log) {
            ...
            if (log.length <= 0) {
                ...
                for (x in log) {
                    ...
                    var params = log[x].split(":::"),
                        msg = "Players: {0} and {1} -- Winner: {2} -- Forfeit: {3} -- Tier: {4} -- Time: {5} -- {0}'s IP: {6} -- {1}'s IP: {7}";
    ...
    if (command === "cookieban" || command === "cookiemute") {
        ...
        if ((sys.os(tarId) !== "android" && sys.version(tarId) < 2402) || (sys.os(tarId) === "android" && sys.version(tarId) < 37)) {
    ...
    if (isSuperAdmin(src)) {
        if (["changeauth"].contains(command)) {
