    ...
    try {
        ...
        if (file === undefined) {
            ...
            sys.webCall(url, function (resp) {
                if (resp !== "") {
                    try {
                        var test = JSON.parse(resp), res = setlint(test, false);
...
function factoryCommand(src, command, commandData, channel) {
    ...
    if (command === "updateteams") {
        ...
        var tmp = commandData.split(":", 2), tier = find_tier(tmp[0]);
        if (tier === null) {
            bfbot.sendMessage(src, "No such tier on server.", channel);
            return;
        }
        ...
        if (createEntry(tier, template, "No URL for addtier")) {
            autoSave("teams", tier);
            sendChanAll("Added the tier " + tier + "!", teamrevchan);
            refresh(tier);
            reviewers[tier] = [];
            sys.writeToFile(submitDir + "reviewers.json", JSON.stringify(reviewers));
        } else {
        ...
        var url, tmp = commandData.split(" ~ ", 2);
        if (tmp.length !== 2) {
        ...
        if (find_tier(commandData) === null) {
            bfbot.sendMessage(src, commandData + " tier doesn't exists on this server.", channel);
            return;
        }
        commandData = find_tier(commandData);
        if (!userqueue.hasOwnProperty(commandData) || userqueue[commandData].length === 0) {
            bfbot.sendMessage(src, "Nothing in the " + commandData + " tier queue.", channel);
            return;
        }
...
module.exports = {
    handleCommand: function(source, message, channel) {
        var command, commandData = "", pos = message.indexOf(" ");
        if (pos !== -1) {
