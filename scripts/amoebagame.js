    ...
    var beforeChatMessage = function (src, message, chan) {
        ...
        if (role != species.length - 1) {
            ...
            if (battles[role] && battles[role] != src && sys.isInChannel(battles[role], id)) {
                ...
                sendChanAll("+EvolutionBot: Two " + species[role] + "s, " + sys.name(battles[role]) + " and " + sys.name(src) + " engaged in a fierce battle! " + sys.name(winner) + " won and is now a " + species[SESSION.users(winner).amoeba.role] + ". " + sys.name(loser) + " lost and is now a " + species[SESSION.users(loser).amoeba.role] + ".", id);
                delete battles[role];
            } else {
                battles[role] = src;
            }
