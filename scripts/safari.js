...
function Safari() {
    ...
    */
    ...
    this.throwBall = function(src, data, bypass, suppress) {
        ...
        var costumeMod = 1;
        if (player.costume === "preschooler" && leader === player.starter) {
    ...
    this.fightTower = function(src, data) {
        ...
        if (contestCount > 0) {
            safaribot.sendMessage(src, "You cannot start this challenge during a contest!", safchan);
            return;
        }
        if (currentPokemon) {
            safaribot.sendMessage(src, "You cannot start a battle while a wild Pokémon is around!", safchan);
    ...
    this.showItemHelp = function (src, data) {
        ...
        //TODO: probably convert this to autopull... but lazy for now
        var costumeHelp = {
            breeder: costumeData.breeder.effect,
            preschooler: costumeData.preschooler.effect,
            pokefan: costumeData.pokefan.effect,
