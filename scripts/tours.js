...
var autotiers = ["Challenge Cup", "CC 1v1", "Wifi CC 1v1", "SM Ubers", "SM OU", "Battle Factory 6v6", "Monotype", "SM 1v1", "Hackmons Challenge Cup", "Inverted Challenge Cup"];
    ...
    try {
        ...
        if (isMegaUser(src)) {
            if (command == "tour" || command == "shift" || ((command == "tourstart" && isTourOwner(src)))) {
                ...
                var selftours = 0;
                var maxtours = sys.auth(src) > 0 ? 6 : 4;
                if (cqueue.length > 0) {
                    for (var c in cqueue) {
        ...
        if (command == "join") {
            ...
            if (sys.os(src) === "android") { // band aid fix
