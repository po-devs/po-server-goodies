...
var tourcommands = ["/join: Joins a tournament.", "/unjoin: Unjoins a tournament during signups only.", "/queue: Lists upcoming tournaments.", "/viewround: Views current round.", "/iom: Views list of ongoing matches.", "/ipm: Views list of matches yet to start.", "/history: Views recently played tiers.", "/megausers: Lists all users that can start tournaments.", "/leaderboard [tier]: Shows tournament rankings, tier is optional.", "/eventleaderboard [month]: Shows the event leaderboard (month is optional).", "/monthlyleaderboard [month]: Shows tour rankings for the current month.", "/tourinfo: Gives information on a person's recent tour wins.", "/eventinfo: Gives information on a person's recent event wins.", "/viewstats: Views tournament stats.", "/viewseeds [tier]: Views seed rankings for that tier.", "/activemu: Lists active megausers.", "/tourrules: Lists the tournament rules.", "/touralerts [on/off]: Turn on/off your tour alerts (Shows list of Tour Alerts if on/off isn't specified).", "/addtouralert: Adds a tour alert for the specified tier.", "/removetouralert: Removes a tour alert for the specified tier."];
var tourmodcommands = ["Parameters: They can be used by putting 'gen=x'; 'mode=singles/doubles/triples'; 'type=single/double'; 'wifi=on/off'.", "For example: '/tour Challenge Cup:gen=RBY:mode=triples:type=double:wifi=on' starts a RBY Challenge Cup double elimination tournament (in Triples mode) with Team Preview.", "/tour: Puts a tour in the queue. Format is /tour tier:parameters.", "/tourmute: Tourmutes a problematic player. Format is /tourmute player:reason:time.", "/tourunmute: Untourmutes a player.", "/tourmutes: Lists tour mutes.", "/endtour: Ends the tour of the specified tier.", "/sub: Subs in a player for another in a tournament. Format is /sub newname:oldname.", "/dq: disqualifies a player.", "/remove [tour/number]: Removes a tournament from the queue. If a number is put in, it will remove the tour in the queue with the corresponding number. If a tier is put in, it will remove the tournament of that tier (starting from the back).", "/cancelbattle: Cancels a player's current battle.", "/start: Starts next tournament in the queue immediately (use sparingly).", "/viewstaffstats: Views tournament staff stats for a user.", "/shift: Places a tournament in the front of the queue. Format is /shift tier:parameters.", "/passmu: To give your Mega User powers to an alt."];
var tourownercommands = ["/megauser: Makes someone a megauser. Use /smegauser for a silent promotion.", "/megauseroff: Fires someone from being tournament authority. Use /smegauseroff for a silent demotion.", "/forcestart: Ends signups immediately and starts the first round.", "/muhistory [days]: Views the activity of megausers (days is optional, if excluded it will get the last 7 days if possible).", "/stopautostart: If there are no tournaments running, this will stop new ones from being automatically started by the server until another one is started manually.", "/tourstart: Starts a tour of that tournament immediately, provided one is not in signups. Format is /tourstart tier:parameters.", "/clearrankings [all/month]: Clears the tour rankings, 'all' clears all history, [month] will only clear a particular month (eg /clearrankings January).", "/cleareventrankings [all/month]: Clears the event rankings, 'all' clears all history, [month] will only clear a particular month (eg /clearrankings January).", "/clearmetric: Clears staff stats and tour history.", "/rundecay: Forces a calculation of seed rankings.", "/addrangewarning: Adds a warning for an IP range.", "/removerangewarning: Removes a warning for an IP range.", "/rangewarns: Checks the current IP range warnings.", "/evalvars: Checks the current variable list for tours.", "/config: Shows config settings.", "/configset: Changes config settings. Format is /configset var:value.", "/resettours: Resets the entire tournament system in the event of a critical failure.", "/starttours: Reverts effect of /stoptours.", "/stoptours: Stops the tournament system for maintenance.", "/fullleaderboard: Gives the full leaderboard for a specified tier.", "/fullmonthlyleaderboard: Gives the full monthly leaderboard for a specified month.", "/loadevents: Load event tours."];
var serverownercommands = ["/tourowner: Makes someone a Tournament Owner. Use /stowner for a silent promotion."];
var tourrules = ["*** TOURNAMENT GUIDELINES ***", "Breaking the following rules may result in punishment:", "#1: Team revealing or scouting in tiers other than Challenge Cup, Battle Factory, or Metronome will result in disqualification.", "- Scouting is watching the battle of someone else in the tournament to gain information.", "- Team revealing is revealing any information about other entrants' teams.", "- Players are always permitted to watch the final match of any tournament.", "#2: Have a team and be ready when you join, otherwise you can be disqualified.", "#3: Tierspamming, repeatedly asking for tournaments in the chat, is not allowed.", "#4: Do not abuse the tournament commands.", "#5: Do not leave or forfeit in a tournament you are in just so you can join another or to give your opponent a 'free' win.", "#6: Do not timestall (i.e. deliberately wait until timeout).", "#7: Inactive/Idle players will automatically be disqualified.", "- Post a message and make sure you are not idle, otherwise you risk being disqualified.", "#8: If there is a problem with your match, contact a megauser as soon as possible.", "- Deliberately drawing your matches using the Suggest Draw button is not permitted unless you have permission from a megauser to restart.", "- Your team is expected to be ready before the match starts - loading the wrong team is not a valid reason to restart.", "#9: Ask someone on the /activemu list if you need help or have problems.", "#10: Event tournaments (marked by red borders)", "- Be aware that these are all double elimination. Breaking or attempting to break the above rules will result in immediate disqualification.", "#11: Respect other players, sportsmanship, and integrity:", "- Avoid complaining about hax, luck or other such things as much as possible.", "- Avoid making inflammatory remarks/taunts towards other users - treat other users the way you would like to be treated.", "- Any deliberate attempt to undermine the integrity of tournaments will result in a permanent ban from tournaments.", "#12: Do not join multiple tours even if you are using a different alt.", "#13: Do not attempt to circumvent the rules.", "- Attempting to circumvent the rules through trickery, proxy or other such methods will be punished."];
var tierToRmtId = {
    "XY OU": 42,
    "XY Ubers": 43,
    "XY UU": 44,
...
function sendDebugMessage(message, chan) {
    if (chan === tourschan && typeof tourconfig.debug == "string" && sys.existChannel(sys.channel(tourserrchan))) {
        if (sys.id(tourconfig.debug) !== undefined && sys.isInChannel(sys.id(tourconfig.debug), tourserrchan)) sendBotMessage(sys.id(tourconfig.debug), message, tourserrchan, false);
    }
}
...
function getConfigValue(file, key) {
    try {
        ...
        if (configkeys.indexOf(key) == -1) {
            ...
            if (defaultvars.hasOwnProperty(key)) return defaultvars[key];
            else throw "Couldn't find the key!";
        }
        else {
...
function getEventTour(datestring) {
    ...
    for (var x in events) {
        if (x == datestring) {
            ...
            var parameters = {
                "gen": "default",
                "mode": modeOfTier(thetier),
                "type": "double",
                "maxplayers": false,
                "event": true,
                "wifi": sys.getClauses(thetier) % 256 >= 128 ? true : false
            };
    ...
    if (canstart && tours.globaltime > 0 && tours.globaltime <= parseInt(sys.time(), 10) && (tourconfig.maxrunning > tours.keys.length || canautostart)) {
        if (tours.queue.length > 0) {
            ...
            if (params.event && tours.keys.length > 0) {
                if ([3, 9, 15, 21].indexOf(hour) == -1) return;
            }
            ...
            tourstart(tourtostart, "~~Server~~", tours.key, {
                "mode": modeOfTier(tourtostart),
                "gen": (allgentiers.indexOf(tourtostart) != -1 ? defaultgen : "default"),
                "type": tourtype,
                "maxplayers": false,
                "event": false,
                "wifi": sys.getClauses(tourtostart) % 256 >= 128 ? true : false
            });
        }
    }
}
...
function tourBattleStart(src, dest, clauses, rated, mode, bid) {
    ...
    if (validBattle && tours.tour[key].round >= 1) {
        tours.tour[key].battlers[name1] = {
            'battleId': bid,
            'time': parseInt(sys.time(), 10),
            'noSpecs': clauses % 8 >= 4
        };
        tours.tour[key].battlers[name2] = {
            'battleId': bid,
            'time': parseInt(sys.time(), 10),
            'noSpecs': clauses % 8 >= 4
        };
...
function tourBattleEnd(winner, loser, result) {
    ...
    if (key === null) return; /* For tournament matches */
    ...
    try {
        ...
        if (isMegaUser(src)) {
            if (command == "tour" || command == "shift" || ((command == "tourstart" && isTourOwner(src)))) {
                ...
                var parameters = {
                    "gen": "default",
                    "mode": modeOfTier(tourtier),
                    "type": detiers.indexOf(tourtier) == -1 ? "single" : "double",
                    "maxplayers": false,
                    "event": false,
                    "wifi": (sys.getClauses(tourtier) % 256 >= 128 ? true : false)
                };
                if (data[1] !== false) {
...
/*if (sys.auth(src) === 0) {
            ...
            if (command == "tourmute") {
                ...
                tours.tourmutes[ip] = {
                    'expiry': parseInt(sys.time(), 10) + time,
                    'reason': reason,
                    'auth': sys.name(src),
                    'name': tar.toLowerCase(),
            ...
            for (var y in tours.tour) {
                ...
                for (var x = 0; x < tours.tour[y].players.length; x += 2) {
                    if (winners.indexOf(tours.tour[y].players[x]) != -1 && tours.tour[y].players[x] != "~Bye~" && tours.tour[y].players[x] != "~DQ~") {
                        if (command == "viewround") roundtable = roundtable + "<tr><td align='right'><font color=green><b>" + toTourName(tours.tour[y].players[x]) + "</b></font></td><td align='center'> won against </td><td>" + toTourName(tours.tour[y].players[x + 1]) + "</td></tr>";
                    }
                    else if (winners.indexOf(tours.tour[y].players[x + 1]) != -1 && tours.tour[y].players[x + 1] != "~Bye~" && tours.tour[y].players[x + 1] != "~DQ~") {
                        if (command == "viewround") roundtable = roundtable + "<tr><td align='right'><font color=green><b>" + toTourName(tours.tour[y].players[x + 1]) + "</b></font></td><td align='center'> won against </td><td>" + toTourName(tours.tour[y].players[x]) + "</td></tr>";
                    }
                        ...
                        if (command != "iom") roundtable = roundtable + "<tr><td align='right'>" + toTourName(tours.tour[y].players[x]) + "</td><td align='center'> VS </td><td>" + toTourName(tours.tour[y].players[x + 1]) + "</td></tr>";
                    }
                }
...
function removesubs(key) {
    try {
        ...
        toDq.forEach(function (player) {
            disqualify(player, key, true, false);
        });
...
function battleend(winner, loser, key) {
    try {
        var winname = sys.name(winner).toLowerCase();
        var losename = sys.name(loser).toLowerCase(); /* We need to check if winner/loser is in the Winners List */
            ...
            if (winner !== "~Bye~") {
                ...
                if (!isevent) {
                    ...
                    for (var p = 0; p < rankingorder.length; p++) {
                        if (rankingorder[p] != "~DQ~" && rankingorder[p] != "~Bye~") tstats.addtourpoints(rankingorder[p], tours.tour[key].cpt, tours.tour[key].tourtype, tours.tour[key].parameters.type == "double" ? true : false, p + 1);
                    }
                }
            }
...
function isValidTourBattle(src, dest, clauses, mode, team, destTier, key, challenge) { // challenge is whether it was issued by challenge
    try {
        ...
        else if (tours.tour[key].state == "final" && clauses % 8 >= 4) { /* We allow Disallow Spects to be used for all rounds except finals */
