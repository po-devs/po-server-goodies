    ...
    if (command == "userinfo" || command == "whois" || command == "whereis") {
        ...
        if (command == "userinfo") {
            ...
            var whois = function(resp) {
                ...
                if (online) {
                    ...
                    if (sys.os(tar) === "windows" || sys.os(tar) === "mac" || sys.os(tar) === "linux") {
                        ...
                        var verArr = ["7.2.1", "7.2", "7.1", "7.0", "6.3","6.1","6.0","5.2","5.1","5.0","4.4"];
                    ...
                    if (sys.auth(src) > 1) {
                        data.push("Unique ID: " + (sys.uniqueId(tar) ? sys.uniqueId(tar).id : "Unknown"));
                    }
                }
                if (authLevel > 0) {
            ...
            if (command === "whereis" && ipcheck) {
                ...
                sys.webCall('https://api.ipinfodb.com/v3/ip-city/?key=' + ipApi + '&ip='+ ip + '&format=json', whois);
            } else {
                whois();
            }
        }
