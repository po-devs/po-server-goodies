...
function POChannelManager(fname) { /* Permanent channels */
...
POChannelManager.prototype.copyAttrs = ["topic", "topicSetter", "perm", "members", "operators", "admins", "masters", "inviteonly", "muteall", "meoff", "muted", "banned", "ignorecaps", "ignoreflood", "rules"];
...
POChannelManager.prototype.restoreSettings = function (channel) {
    ...
    this.copyAttrs.forEach(function (attr) {
        if (chanData !== null && chanData.hasOwnProperty(attr)) chan[attr] = chanData[attr];
    });
};
