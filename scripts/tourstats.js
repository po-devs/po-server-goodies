...
function updateTourStats(tier, time, winner, num, noPoints, purgeTime) {
    ...
    if (purgeTime === undefined) purgeTime = 60 * 60 * 24 * 31; // 31 days
...
var commandHelp = ["/viewtiers: Shows the recently played tournaments, which can't be started currently.", "/tourrankings: Shows recent tournament winners.", "/tourranking [tier]: Shows recent tourney winners in a specific tier.", "/tourdetails [name]: Shows a user's tourney stats.", "/lastwinners: Shows details about recent tournaments.", "/writetourstats: Forces a writing of tour stats to tourstats.json.", "/reloadtourstats: Forces a reload of tour stats from tourstats.json.", "/resettourstats: Resets tournament winners."];
...
exports.getRankingsByTier = function () {
    return tourrankingsbytier;
}
