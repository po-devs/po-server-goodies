...
function () {
    ...
    var ownerCommands = {
        ...
        'getresults': function (src, commandData) {
            ...
            if (verbose) normalbot.sendChanMessage(src, "Following people voted:") for (var i = 0; i < total_users; ++i) {
                var voter = poll.voters[i];
                if (verbose) normalbot.sendChanMessage(src, "" + voter.name + " (" + (voter.ip ? voter.ip : "unknown ip") + ") voted for " + voter.answer + " with rating " + voter.rating + ". Approved: " + voter.whitelisted);
