exports.ownerCommands = function(src, command, commandData, tar) {
    if (command == "ipban") {
        ...
        if (!correct) {
            bots.normal.sendChanMessage(src, "The IP address looks strange, you might want to correct it: " + subip);
            return;
        }
        ipbans.add(subip, "Name: " +sys.name(src) + " Comment: " + rangebans.escapeValue(comment));
        bots.normal.sendChanAll("IP ban added successfully for IP subrange: " + subip + " by "+ sys.name(src),staffchannel);
        return;
    }
    if (command == "ipunban") {
        ...
        if (ipbans.get(subip) !== undefined) {
            ipbans.remove(subip);
            bots.normal.sendChanMessage(src, "IP ban removed successfully for IP subrange: " + subip);
        } else {
            bots.normal.sendChanMessage(src, "No such IP ban.");
        }
        return;
    }
    if (command == "changerating") {
        ...
        if (data.length != 3) {
            bots.normal.sendChanMessage(src, "You need to give 3 parameters.");
            return;
        }
        var player = data[0];
        var tier = data[1];
        ...
        bots.normal.sendChanMessage(src, "Rating of " + player + " in tier " + tier + " was changed to " + rating);
        return;
    }
    ...
    if (command == "capslockday") {
        if (commandData == "off") {
            ...
            bots.normal.sendChanMessage(src, "You turned caps lock day off!");
        }
        else if (commandData == "on") {
            CAPSLOCKDAYALLOW = true;
            bots.normal.sendChanMessage(src, "You turned caps lock day on!");
        }
        return;
    }
    if (command == "contributor") {
        ...
        if (sys.dbIp(name) === undefined) {
            bots.normal.sendChanMessage(src, name + " couldn't be found.");
            return;
        }
        bots.normal.sendChanMessage(src, name + " is now a contributor!");
    ...
    if (command == "contributoroff") {
        ...
        if (contrib === "") {
            bots.normal.sendChanMessage(src, commandData + " isn't a contributor.", channel);
            return;
        }
        contributors.remove(contrib);
        bots.normal.sendChanMessage(src, commandData + " is no longer a contributor!");
        return;
    }
    if (command == "showteam") {
        ...
        if (teams) {
            ...
            bots.normal.sendAll(sys.name(src) + " just viewed " + sys.name(tar) + "'s team.", staffchannel);
        } else {
            bots.normal.sendChanMessage(src, "That player has no teams with valid pokemon.");
        }
        return;
    }
    if (command == "rangeban") {
        ...
        bots.normal.sendChanMessage(src, "Rangeban added successfully for IP subrange: " + subip);
        /* kick them */
    ...
    if (command == "rangeunban") {
        ...
        if (rangebans.get(subip) !== undefined) {
            ...
            bots.normal.sendChanMessage(src, "Rangeban removed successfully for IP subrange: " + subip);
        } else {
            bots.normal.sendChanMessage(src, "No such rangeban.");
        }
        return;
    }
    if (command == "purgemutes") {
        ...
        if (removed.length > 0) {
            bots.normal.sendChanMessage(src, "" + removed.length + " mutes purged successfully.");
        } else {
            bots.normal.sendChanMessage(src, "No mutes were purged.");
        }
        return;
    }
    if (command == "purgembans") {
        ...
        if (removed.length > 0) {
            bots.normal.sendChanMessage(src, "" + removed.length + " mafiabans purged successfully.");
        } else {
            bots.normal.sendChanMessage(src, "No mafiabans were purged.");
        }
        return;
    }
    if (command == "sendall") {
    ...
    if (command == "imp") {
        ...
        bots.normal.sendChanMessage(src, "Now you are " + SESSION.users(src).impersonation + "!");
        return;
    }
    if (command == "impoff") {
        delete SESSION.users(src).impersonation;
        bots.normal.sendChanMessage(src, "Now you are yourself!");
        return;
    }
    if (command == "autosmute") {
        ...
        if (autosmute.indexOf(name) !== -1) {
            bots.normal.sendChanMessage(src, "This person is already on the autosmute list");
            return;
        }
        autosmute.push(name);
        ...
        bots.normal.sendAll(commandData + " was added to the autosmute list", staffchannel);
        return;
    }
    if (command == "removeautosmute") {
        ...
        autosmute = autosmute.filter(function(list_name) {
            if (list_name == name) {
                bots.normal.sendAll(commandData + " was removed from the autosmute list", staffchannel);
                return true;
            }
        });
        sys.writeToFile('secretsmute.txt', autosmute.join(":::"));
    ...
    if (command == "periodicsay" || command == "periodichtml") {
        ...
        var callback = function(sayer, minutes, cids, what, count) {
            ...
            if (SESSION.users(sayer).endcalls) {
                bots.normal.sendMessage(src, "Periodic say of '"+what+"' has ended.");
        ...
        bots.normal.sendMessage(src, "Starting a new periodicsay");
    ...
    if (command == "endcalls") {
        if (SESSION.users(src).callcount === 0 || SESSION.users(src).callcount === undefined) {
            bots.normal.sendMessage(src, "You have no periodic calls I think.");
        } else {
            bots.normal.sendMessage(src, "You have " + SESSION.users(src).callcount + " calls running.");
        }
        if (SESSION.users(src).endcalls !== true) {
            SESSION.users(src).endcalls = true;
            bots.normal.sendMessage(src, "Next periodic call called will end.");
        } else {
            SESSION.users(src).endcalls = false;
            bots.normal.sendMessage(src, "Cancelled the ending of periodic calls.");
        }
        return;
    }
    if (command == "changeauth" || command == "changeauths") {
        ...
        if (newauth > 0 && !sys.dbRegistered(name)) {
            bots.normal.sendMessage(src, "This person is not registered");
            bots.normal.sendMessage(tar, "Please register, before getting auth");
            return;
        }
        if (tar !== undefined) sys.changeAuth(tar, newauth);
        else sys.changeDbAuth(name, newauth);
        if (!silent) bots.normal.sendAll("" + sys.name(src) + " changed auth of " + name + " to " + newauth);
        else bots.normal.sendAll("" + sys.name(src) + " changed auth of " + name + " to " + newauth, staffchannel);
        return;
    }
    ...
    if (command == "clearladder") {
        ...
        if(tier) {
            ...
            bots.normal.sendAll(tier + " ladder has been reset!");
            return;
        }
        bots.normal.sendMessage(src, commandData + " is not a tier");
        return;
    }
    if (command == "indigo") {
        if (commandData == "on") {
            ...
            bots.normal.sendMessage(src, "Staff channel was remade!");
            return;
        }
        if (commandData == "off") {
            ...
            bots.normal.sendMessage(src, "Staff channel was destroyed!");
            return;
        }
    }
    if (command == "stopbattles") {
        ...
        if (battlesStopped)  {
            sendChanAll("", -1);
            sendChanAll("*** ********************************************************************** ***", -1);
            bots.battle.sendAll("The battles are now stopped. The server will restart soon.");
            sendChanAll("*** ********************************************************************** ***", -1);
            sendChanAll("", -1);
        } else {
            bots.battle.sendAll("False alarm, battles may continue.");
        }
        return;
    }
    if (command == "clearpass") {
        ...
        bots.normal.sendChanMessage(src, "" + commandData + "'s password was cleared!");
        if (tar !== undefined) {
            bots.normal.sendMessage(tar, "Your password was cleared by " + mod + "!");
    ...
    if (command == "updatenotice") {
        updateNotice();
        bots.normal.sendMessage(src, "Notice updated!");
        return;
    }
    if (command == "updatebansites") {
        bots.normal.sendChanMessage(src, "Fetching ban sites...");
        sys.webCall(Config.base_url + "bansites.txt", function(resp) {
            if (resp !== "") {
                ...
                bots.normal.sendAll('Updated banned sites!', staffchannel);
            } else {
                bots.normal.sendAll('Failed to update!', staffchannel);
            }
        });
        return;
    }
    if (command == "updatetierchecks"){
        ...
        bots.normal.sendAll('Updated tier checks!', staffchannel);
        return;
    }
    if (command == "updatescripts") {
        bots.normal.sendChanMessage(src, "Fetching scripts...");
        ...
        var changeScript = function(resp) {
            ...
            try {
                ...
                bots.normal.sendAll('Updating failed, loaded old scripts!', staffchannel);
                sys.sendMessage(src, "ERROR: " + err + (err.lineNumber ? " on line: " + err.lineNumber : ""), channel_local);
                print(err);
            }
        };
        bots.normal.sendChanMessage(src, "Fetching scripts from " + updateURL);
        sys.webCall(updateURL, changeScript);
        return;
    }
    if (command == "updatetiers" || command == "updatetierssoft") {
        bots.normal.sendChanMessage(src, "Fetching tiers...");
        var updateURL = Config.base_url + "tiers.xml";
        if (commandData !== undefined && (commandData.substring(0,7) == 'http://' || commandData.substring(0,8) == 'https://')) {
            updateURL = commandData;
        }
        bots.normal.sendChanMessage(src, "Fetching tiers from " + updateURL);
        var updateTiers = function(resp) {
            ...
            try {
                ...
                if (command == "updatetiers") {
                    ...
                    bots.normal.sendMessage(src, "Tiers.xml updated!", channel);
                }
            } catch (e) {
                bots.normal.sendChanMessage(src, "ERROR: "+e);
                return;
            }
        };
    ...
    if (command == "addplugin") {
        ...
        bots.normal.sendChanMessage(src, "Downloading module " + commandData + "!");
        return;
    }
    if (command == "removeplugin") {
        ...
        for (var i = 0; i < POglobal.plugins.length; ++i) {
            if (commandData == POglobal.plugins[i].source) {
                bots.normal.sendChanMessage(src, "Module " + POglobal.plugins[i].source + " removed!");
                POglobal.plugins.splice(i,1);
                return;
            }
        }
        bots.normal.sendChanMessage(src, "Module not found, can not remove.");
        return;
    }
    if (command == "updateplugin") {
        ...
        var MakeUpdateFunc = function(i, source) {
            return function(module) {
                ...
                module.init();
                bots.normal.sendMessage(src, "Module " + source + " updated!", bind_channel);
            };
        };
        for (var i = 0; i < POglobal.plugins.length; ++i) {
            if (commandData == POglobal.plugins[i].source) {
                ...
                bots.normal.sendChanMessage(src, "Downloading module " + source + "!");
                return;
            }
        }
        bots.normal.sendChanMessage(src, "Module not found, can not update.");
        return;
    }
    if (command == "loadstats") {
        if (sys.loadServerPlugin("serverplugins/libusagestats.so")) {
            bots.normal.sendChanMessage(src, "Usage Stats plugin loaded");
            return;
        }
        bots.normal.sendChanMessage(src, "Usage Stats failed to load");
        return;
    }
    if (command == "unloadstats") {
        if (sys.unloadServerPlugin("Usage Statistics")){
            bots.normal.sendChanMessage(src, "Usage Stats plugin unloaded");
            return;
        }
        bots.normal.sendChanMessage(src, "Usage stats failed to unload");
        return;
    }
